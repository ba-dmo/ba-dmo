...
public sealed class AccessResolver
{
    ...
    /// added by the resolver, never by role-name branching.
    /// </summary>
    public EffectiveAccess Resolve(AccessTemplateDefinition template)
    {
        ...
        // operational tabs and Administrators.
        if (_catalog.ContainsModule(CanonicalModuleCatalog.JobonModuleId))
        {
    ...
    /// authenticated user → Job On landing, identical for every template;
    /// no role/template-specific landing. Only when Job On is genuinely
    /// unavailable under the canonical contract (landing page absent/inactive,
    /// or not accessible) does the deterministic fallback apply: first
    /// accessible page in canonical display order. With no accessible page
    /// the result is the explicit NoAccess state (GLM-SHL-06).
