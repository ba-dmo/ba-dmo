...
public class SupabaseAuthAdapterTests
{
    ...
    [Fact]
    public async Task InvalidCredentials_FailWithGenericMessage()
    {
        var handler = new FakeHttpMessageHandler();
        handler.Responders.Enqueue(_ => Json(
        ...
        // Never echo provider details or disclose email existence.
        Assert.DoesNotContain("user not found", result.Error.Message, StringComparison.Ordinal);
    }
