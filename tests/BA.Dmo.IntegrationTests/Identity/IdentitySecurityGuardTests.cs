...
{
    [Fact]
    public void ProvisioningAdapter_IsOnlyUsedByTheBootstrapCli_NotByPages()
    {
        ...
        Assert.True(offenders.Count == 0,
            "IAdminProvisioningAdapter (service_role) must exist only in the bootstrap-admin " +
            $"CLI path. Offenders: {string.Join("; ", offenders)}");
    }
