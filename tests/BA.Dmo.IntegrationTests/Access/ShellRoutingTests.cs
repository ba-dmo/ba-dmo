...
public class ShellRoutingTests : IClassFixture<ShellRoutingTests.ShellFixture>
{
    ...
    [Fact]
    public async Task Scenario7_AdminWithoutOperationalModules_LandsOnJobOn_AndReachesAdminByNavigation()
    {
        _fixture.Profile = ShellFixture.UserProfile.AdminOnly;
        var client = await LoginAsync();

        // UD-16: the administrator also lands on Job On — never /admin.
        var home = await client.GetAsync("/");
        Assert.Equal("/jobon", home.Headers.Location!.ToString());
        ...
        var shellHtml = await (await client.GetAsync("/jobon")).Content.ReadAsStringAsync();
        AssertNav(shellHtml, present: new[] { "jobon", "admin" });
        AssertNav(shellHtml, present: null, absent: new[] { "boquilhas", "controlo" });
    }

    [Fact]
    public async Task Scenario8_AdminWithModules_SeesBoth_AdminGrantsNoImplicitFunctionalAccess()
    {
        _fixture.Profile = ShellFixture.UserProfile.AdminPlusBoquilhas;
        var client = await LoginAsync();

        var shellHtml = await (await client.GetAsync("/jobon")).Content.ReadAsStringAsync();
        AssertNav(shellHtml, present: new[] { "jobon", "boquilhas", "admin" });

        Assert.Equal(HttpStatusCode.OK, (await client.GetAsync("/boquilhas")).StatusCode);
        Assert.Equal(HttpStatusCode.OK, (await client.GetAsync("/admin")).StatusCode);
        // Admin presence grants nothing outside its own module.
        var denied = await client.GetAsync("/ferramentas");
        Assert.StartsWith("/access-denied", denied.Headers.Location!.PathAndQuery);
    }
        ...
        private sealed class FakeIdentityRepository(ShellFixture fixture) : IInternalUserRepository
        {
                ...
                Guid authUserId, CancellationToken cancellationToken = default)
            {
                ...
                var modulesJson = fixture.Profile switch
                {
                    ...
                    UserProfile.AdminPlusBoquilhas =>
                        "[{\"moduleId\":\"admin\",\"capabilities\":[\"admin.gerir\"]},{\"moduleId\":\"boquilhas\",\"capabilities\":[]}]",
