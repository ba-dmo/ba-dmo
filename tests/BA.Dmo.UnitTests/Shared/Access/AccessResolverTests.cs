...
/// GLM-CTR-02, UD-16/DS-01). Landing rule (owner-confirmed design): every
/// authenticated user lands on Job On; no role-name-specific landing;
...
public class AccessResolverTests
{
    ...
    [Fact]
    public void Landing_IsJobOn_ForAnAdminOnlyTemplate()
    {
        // Scenario 7: Admin without operational modules → landing Job On;
        // Administração is reached through navigation. Proves there is no
        // Admin-specific landing shortcut.
        var access = _resolver.Resolve(Template(
